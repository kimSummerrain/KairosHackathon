...
try:
    ...
    for row in results:
        ...
        formatted_time = datetime.strftime(time_value, "%Y-%m")  # 년-월 형식으로 포맷팅합니다.

        if not Dict.get(formatted_time):         # 날짜가 없다면 날짜를 추가
            Dict[formatted_time] = [[],[]]
        
        if row[1] in Dict[formatted_time][0]:       # 중복이 되는가?
            Dict[formatted_time][1].append(row[2])      # 된다면 가격을 리스트에 추가
        else:
            Dict[formatted_time][0].append(row[1]) # 중복이 안되어 있다면
            Dict[formatted_time][1].append(row[2]) # 가격을 리스트에 추가

    month_chart_data=Moving_result.filter(Dict)
    #print(Dict)
