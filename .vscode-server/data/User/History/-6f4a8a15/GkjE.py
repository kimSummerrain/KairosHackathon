...
try:
    ...
    with conn.cursor() as cursor:
        ...
        for row in results:
            ...
            # 날짜가 이미 존재한다면 해당 키값에 값을 추가합니다.
            if formatted_time in Dict:
                Dict[formatted_time][0].append(row[1])
                Dict[formatted_time][1].append(row[2])
            else:  # 날짜가 없다면 날짜를 추가하고 값을 할당합니다.
                Dict[formatted_time].append([[row[1]], [row[2]]])
        print(Dict)
